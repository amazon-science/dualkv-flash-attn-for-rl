...
class ParallelQwen2AttentionRmPad(ParallelQwen2Attention):
    def forward(
        ...
        if dualkv_context is not None:
            # DualKV-SplitQ: loop over prompt groups, each sharing context K/V.
            # For each group: Call 1 (context attention, once) + Call 2 (DualKV decoded).
            # Saves (N-1)*P redundant context attention per group.
            from flash_attn import flash_attn_dualkv_varlen_func

            P = dualkv_context["P"]
            dualkv_cu_seqlens = dualkv_context["cu_seqlens"]
            prompt_group_sizes = dualkv_context["prompt_group_sizes"]

            # Use DualKV cu_seqlens (per-sequence boundaries) instead of model's
            cu_seqlens_dk = dualkv_cu_seqlens
            batch_size = cu_seqlens_dk.shape[0] - 1

            out_parts = []
            seq_offset = 0  # index into cu_seqlens_dk

            for g_size in prompt_group_sizes:
                # Slice this group's cu_seqlens and re-zero
                g_cu = cu_seqlens_dk[seq_offset:seq_offset + g_size + 1]
                g_token_start = g_cu[0].item()
                g_token_end = g_cu[-1].item()
                g_cu_local = g_cu - g_cu[0]  # re-zero for flash_attn

                # This group's Q, K, V
                g_q = query_states[g_token_start:g_token_end]
                g_k = key_states[g_token_start:g_token_end]
                g_v = value_states[g_token_start:g_token_end]

                # Context K/V from first sequence in group (shared prompt)
                k_ctx = g_k[:P]   # (P, nh_kv_per_tp, hd)
                v_ctx = g_v[:P]   # (P, nh_kv_per_tp, hd)

                # Context Q from first sequence
                q_ctx = g_q[:P]   # (P, nh_per_tp, hd)

                # Decoded Q, K, V: response portion of each sequence in group
                q_dec_parts = []
                k_dec_parts = []
                v_dec_parts = []
                decoded_lens = []
                for i in range(g_size):
                    s = g_cu_local[i].item()
                    e = g_cu_local[i + 1].item()
                    q_dec_parts.append(g_q[s + P:e])
                    k_dec_parts.append(g_k[s + P:e])
                    v_dec_parts.append(g_v[s + P:e])
                    decoded_lens.append(e - s - P)

                q_dec = torch.cat(q_dec_parts, dim=0)
                k_dec = torch.cat(k_dec_parts, dim=0)
                v_dec = torch.cat(v_dec_parts, dim=0)

                max_decoded = max(decoded_lens)
                cu_dec = torch.zeros(g_size + 1, device=cu_seqlens_dk.device, dtype=torch.int32)
                for i in range(g_size):
                    cu_dec[i + 1] = cu_dec[i] + decoded_lens[i]

                # Call 1: Context attention (standard FA2, P tokens, once per group)
                cu_ctx = torch.tensor([0, P], device=cu_seqlens_dk.device, dtype=torch.int32)
                ...
                # Bottom-right causal alignment: seqlen_q=R_i, seqlen_k=P+R_i
                dec_out = flash_attn_dualkv_varlen_func(
                    q_dec, k_ctx, v_ctx, k_dec, v_dec,
                    cu_dec, cu_dec,
                    max_seqlen_q=max_decoded,
                    context_seqlen=P,
                    max_seqlen_k_decoded=max_decoded,
                    causal=True,
                )

                # Assemble: broadcast ctx_out to all sequences in group
                dec_offset = 0
                for i in range(g_size):
                    R_i = decoded_lens[i]
                    out_parts.append(ctx_out)
                    out_parts.append(dec_out[dec_offset:dec_offset + R_i])
                    dec_offset += R_i

                seq_offset += g_size

            attn_output_unpad = torch.cat(out_parts, dim=0)
        else:
